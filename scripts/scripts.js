 ...
 function pogoda() {
     navigator.geolocation.getCurrentPosition(function (position, info) {
         ...
         $.getJSON(api, function (data) {
             ...
             if ((zachmurzenie >= 0) && (zachmurzenie < 21)) {
                 terazZachmurzenie = zachmurzenieNazwa[0]
             }
             if (zachmurzenie >= 21 && zachmurzenie < 41) {
                 terazZachmurzenie = zachmurzenieNazwa[1];
             };
             if (zachmurzenie >= 41 && zachmurzenie < 61) {
                 terazZachmurzenie = zachmurzenieNazwa[2];
             };
             if (zachmurzenie >= 61 && zachmurzenie < 81) {
                 terazZachmurzenie = zachmurzenieNazwa[3];
             };
             if (zachmurzenie >= 81 && zachmurzenie < 100) {
                 terazZachmurzenie = zachmurzenieNazwa[4];
             };
             // opis slowny cisnienia 
             const cisnienieNazwa = ["niskie ciśnienie", "umiarkowane ciśnienie", "wysokie ciśnienie"];
             let terazCisnienie = "";
             if (cisnienie > 950 && cisnienie <= 990) {
                 terazCisnienie = cisnienieNazwa[0]
             }
             if (cisnienie > 991 && cisnienie <= 1005) {
                 terazCisnienie = cisnienieNazwa[1]
             }
             if (cisnienie > 1006 && cisnienie <= 1050) {
                 terazCisnienie = cisnienieNazwa[2]
             }
             console.log("");
